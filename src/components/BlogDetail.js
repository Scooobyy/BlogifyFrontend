...
const BlogDetail = () => {
  ...
  useEffect(() => {
    if (!loading) {
      // Try to find in real blogs first
      const foundBlog = blogs.find(b => b._id === id);
      if (foundBlog) {
        setBlog(foundBlog);
      } else {
        // If not found, try dummy blogs
        const foundDummy = dummyBlogs.find(b => b._id === id);
        if (foundDummy) {
          setBlog(foundDummy);
        } else {
          // Not found anywhere → redirect to dashboard
          navigate('/dashboard');
        }
      }
    }
  }, [loading, blogs, id, navigate]);
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-6 bg-white rounded shadow">
      ...
      <p className="mt-4 text-gray-600">Status: {blog.status}</p>
    </div>
  );
};
